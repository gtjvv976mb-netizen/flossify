...
| Invoice-based ledger and BIR Invoice | Every payment attached to a treatment line; the document is titled **Invoice** (EOPT Act, RR 7-2024) — the schema already models `invoice_series` and `invoice`; only the marketing copy still said "receipt", now fixed — with structured e-invoice ready before 31 Dec 2026 if the clinic is above the micro threshold [43]. | MVP | Exists |
...
- **BIR: the document is an Invoice.** Since 27 April 2024 the Invoice is the primary document for services; ORs are supplementary [43]. The data model already says invoice; keep every screen, print and sentence saying it too, so a clinic never prints a thousand "official receipts" the Bureau no longer wants.
